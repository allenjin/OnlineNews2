...
public class OtherListBean {
	public static final String HTTPCustomer =
			UsefulStaticValues.getIpAddress()+":8080/News2/servlet/OtherListServlet";
	...
	public void getPassageListAsync(final int id,final int page,final CallBack<List<Passage>> callback){
		Thread t = new Thread(){
			@Override
			public void run() {
				super.run();
				try {
					callback.onFinish(getPassageList(id,page));
				} catch (Exception e) {
